...
const Leaves = () => {
  ...
  useEffect(() => {
    const fetchLeaves = async () => {
      try {
        const response = await axios.get("https://school-backend-1-2xki.onrender.com/api/admin/leaves");
        setLeaveList(response.data.leaves || []);
      } catch (error) {
        console.error("Error fetching leaves:", error);
        toast.error("Error fetching leaves. Please try again.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStatusChange = async (leaveId, newStatus) => {
    try {
      const response = await axios.put(`https://school-backend-1-2xki.onrender.com/api/admin/approve-leave/${leaveId}`, {
        status: newStatus,
      });
      ...
      toast.error("Error updating leave status. Please try again.");
    }
  };
    ...
    <div className="min-h-screen flex bg-gray-100">
      ...
      <div className={`flex-grow overflow-y-auto transition-all duration-300 ${isSidebarOpen ? "ml-64" : "ml-0"}`}>
        ...
        <div className="flex items-center justify-between bg-purple-700 text-white p-4 shadow-lg lg:hidden">
          <h1 className="text-lg font-bold">Leave List</h1>
              ...
              {currentLeaves.length > 0 ? (
                currentLeaves.map((leave, index) => (
                  <tr key={leave._id}>
                    ...
                    <td className="px-4 py-2 border-b">{leave.studentId}</td>
                    <td className="px-4 py-2 border-b">{leave.parentId}</td>
                    <td className="px-4 py-2 border-b">{formatDate(leave.createdAt)}</td>
                  </tr>
                ))
              ) : (
                <tr>
